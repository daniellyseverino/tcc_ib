...
#modelo_stan =  stan_model("izabel\\modelLogisticBinomial.stan")

stanBinoNeg = function(stanModel, y){
...
# estimativas = apply(dados_dengue, 2, function(y) stanBinoNeg(modelo_stan, y = y))
# saveRDS(estimativas, "estimativas/binomial/estimativas_n_td_por_delay.rds",version = 2)
estimativas = readRDS("estimativas/binomial/estimativas_n_td_por_delay.rds")

# abcf_por_dalay = lapply(estimativas, function(x) c(mean(x$a), mean(x$b), mean(x$c), mean(x$f)) )
# saveRDS(abcf_por_dalay, "estimativas/binomial/abcf_por_dalay.rds", version = 2)
abcf_por_dalay = readRDS("estimativas/binomial/abcf_por_dalay.rds")

# estimativas_theta = stanBinoNeg(modelo_stan, y = rowSums(dados_dengue, na.rm = TRUE))
# saveRDS(estimativas_theta, "estimativas/binomial/estimativas_theta.rds",version = 2)
estimativas_theta = readRDS("estimativas/binomial/estimativas_theta.rds")
...
modelo_completo_stan =  stan_model("izabel\\modelCompleteLogisticBinomial.stan")
...
saveRDS(estimativas_com_chute, "estimativas\\binomial\\estimativas_com_chute.rds", version = 2)
estimativas_com_chute = readRDS("estimativas\\binomial\\estimativas_com_chute.rds")
